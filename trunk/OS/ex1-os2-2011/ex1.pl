...
#* (argv [1])
#* (argv [2])  Output file to be sent to the numbers (all types) plan call
#* (Argv [3])  Output file to send junk strings
...
open(OUTLET, ">$ARGV[1]") or die("Cannot open file '$ARGV[1]' \n");
open(OUTNOM, ">$ARGV[2]") or die("Cannot open file '$ARGV[2]' \n");
open(OUTGARB, ">$ARGV[3]") or die("Cannot open file '$ARGV[3]'\n");
